  ...
  return (
    <div className="min-h-screen paper-bg">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 pt-10 pb-16 text-center">
        ...
        {/* Floating decorations — hidden on small screens to prevent overflow */}
        <div className="relative mt-12 h-32 sm:h-48 pointer-events-none select-none hidden sm:block">
          {[
            { emoji: '🎈', x: '5%', delay: '0s', size: 'text-4xl sm:text-5xl' },
            { emoji: '✨', x: '18%', delay: '0.5s', size: 'text-2xl sm:text-3xl' },
            { emoji: '🎉', x: '35%', delay: '1s', size: 'text-3xl sm:text-4xl' },
            { emoji: '✉️', x: '52%', delay: '0.3s', size: 'text-5xl sm:text-6xl' },
            { emoji: '🎁', x: '70%', delay: '0.8s', size: 'text-3xl sm:text-4xl' },
            { emoji: '💌', x: '85%', delay: '0.2s', size: 'text-2xl sm:text-3xl' },
            { emoji: '🎊', x: '95%', delay: '1.2s', size: 'text-2xl sm:text-3xl' },
          ].map((item, i) => (
            <span key={i} className={`absolute animate-float ${item.size}`} style={{ left: item.x, top: `${20 + (i % 3) * 30}%`, animationDelay: item.delay }}>
              {item.emoji}
            </span>
          ))}
        </div>
